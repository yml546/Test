...
class MGRUFCell(nn.Module):
    ...
    def forward(self, sample, hiddens, weights):
        ...
        hiddens_filtered = torch.einsum('ijk, ik->ijk', [-hiddens, weights]).sum(dim=0)
        ...
        ############## DO WE NEED AN ACTIVATION FUNCTION HERE ???????
...
class MGRUCell(nn.Module):
    ...
    def forward(self, sample, hiddens, mem_para):
        # mem_para : d_{t-1}
        batch_size = sample.size(0)
        if hiddens is None:
            hiddens = torch.zeros(self.k, batch_size, self.hidden_size,
